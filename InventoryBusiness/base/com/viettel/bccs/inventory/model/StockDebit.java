 ...
 */
@Entity
@Table(name = "STOCK_DEBIT")
@NamedQueries({
    @NamedQuery(name = "StockDebit.findAll", query = "SELECT s FROM StockDebit s")})
public class StockDebit implements Serializable {
public static enum COLUMNS {CREATEDATE, CREATEUSER, DEBITDAYTYPE, DEBITVALUE, FINANCETYPE, LASTUPDATETIME, LASTUPDATEUSER, OWNERID, OWNERTYPE, STOCKDEBITID, EXCLUSE_ID_LIST};
    ...
    @SequenceGenerator(name = "STOCK_DEBIT_ID_GENERATOR", sequenceName = "STOCK_DEBIT_SEQ", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "STOCK_DEBIT_ID_GENERATOR")
    ...
    public StockDebit(Long stockDebitId) {
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof StockDebit)) {
            return false;
        }
        StockDebit other = (StockDebit) object;
