package com.viettel.bccs.inventory.service;

import com.viettel.bccs.inventory.dto.ReportChangeGoodsDTO;
import com.viettel.fw.common.util.extjs.FilterRequest;
import com.viettel.fw.dto.BaseMessage;

import javax.jws.WebMethod;
import javax.jws.WebService;
import java.util.List;

@WebService
public interface ReportChangeGoodsService {

    @WebMethod
    public ReportChangeGoodsDTO findOne(Long id) throws Exception;

    @WebMethod
    public Long count(List<FilterRequest> filters) throws Exception;

    @WebMethod
    public List<ReportChangeGoodsDTO> findAll() throws Exception;

    @WebMethod
    public List<ReportChangeGoodsDTO> findByFilter(List<FilterRequest> filters) throws Exception;

    @WebMethod
    public ReportChangeGoodsDTO create(ReportChangeGoodsDTO reportChangeGoodsDTO) throws Exception;

    @WebMethod
    public BaseMessage update(ReportChangeGoodsDTO productSpecCharacterDTO) throws Exception;

}