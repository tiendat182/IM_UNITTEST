...
/*
thiendn1: override de bo sung tham so kpi
 */
public class CommandButtonRenderer extends CoreRenderer {
    ...
    protected String buildRequest(FacesContext context, CommandButton button, String clientId) throws FacesException {
        ...
        String kpiParamName = "kpi";
        String kpiParamMethod = null;
        String kpiParamValue = null;

        MethodExpression methodExpression = button.getActionExpression();
        if(methodExpression==null){
            javax.faces.event.ActionListener[] actionListeners = button.getActionListeners();
            if (actionListeners != null && actionListeners.length > 0) {
                if(actionListeners[0] instanceof MethodExpressionActionListener){
                    methodExpression = ((MethodExpressionActionListener) actionListeners[0]).getMethodExpressionOneArg();
                }
            }
        }

        if(methodExpression!=null){
            if(methodExpression instanceof TagMethodExpression){
                kpiParamMethod = methodExpression.getExpressionString();
            }
            else if(methodExpression instanceof TagAttributeImpl.AttributeLookupMethodExpression){
                kpiParamMethod = ((TagAttributeImpl.AttributeLookupMethodExpression)methodExpression).getValueExpressionString(FacesContext.getCurrentInstance().getELContext());
            }
        }
        if (kpiParamMethod != null &&kpiParamMethod.length()>3) {
            ResourceBundle resourceBundle = ResourceBundle.getBundle("kpi-mapping", Locale.getDefault());
            if(resourceBundle!=null){
                kpiParamMethod = kpiParamMethod.substring(2, kpiParamMethod.length() - 1);
                try {
                    kpiParamValue = resourceBundle.getString(kpiParamMethod);
                    if (kpiParamValue != null) {
                        UIParameter uiParameter = new UIParameter();
                        uiParameter.setName(kpiParamName);
                        uiParameter.setValue(kpiParamValue);
                        button.getChildren().add(uiParameter);
                    }
