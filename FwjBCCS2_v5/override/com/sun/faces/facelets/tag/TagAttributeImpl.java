...
public class TagAttributeImpl extends TagAttribute {
    ...
    public MethodExpression getMethodExpression(FaceletContext ctx, Class type, Class[] paramTypes) {
        try {
            ExpressionFactory e = ctx.getExpressionFactory();
            if(ELUtils.isCompositeComponentLookupWithArgs(this.value)) {
                String delegate1 = MessageUtils.getExceptionMessageString("com.sun.faces.ARGUMENTS_NOT_LEGAL_WITH_CC_ATTRS_EXPR", new Object[0]);
                throw new TagAttributeException(this, delegate1);
            } else {
                Object result;
                if(ELUtils.isCompositeComponentMethodExprLookup(this.value)) {
                    ...
                    TagMethodExpression delegate = new TagMethodExpression(this, e.createMethodExpression(ctx, this.value, type, paramTypes));
                    result = new ContextualCompositeMethodExpression(this.getLocation(), delegate);
                } else {
                    result = new TagMethodExpression(this, e.createMethodExpression(ctx, this.value, type, paramTypes));
                }
    ...
    public ValueExpression getValueExpression(FaceletContext ctx, String expr, Class type) {
        try {
            ExpressionFactory e = ctx.getExpressionFactory();
            ValueExpression delegate = e.createValueExpression(ctx, expr, type);
    ...
    public static class AttributeLookupMethodExpression extends MethodExpression {
        ...
        public String getValueExpressionString(ELContext elContext) {
            Object object = lookupExpression.getValue(elContext);
            return object==null?null:((ContextualCompositeMethodExpression) object).getExpressionString();
        }
