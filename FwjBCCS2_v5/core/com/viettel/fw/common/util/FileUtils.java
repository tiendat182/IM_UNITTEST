...
//                line = Native2Ascii.asciiToNative(line);
    ...
    public static String getSafePath(String pathName) {
        ...
        do {
            if (!rslt.equals("") && rslt.startsWith(".")) {
    ...
    public static String zipFile(String pathFileInput, String pathFileOutput) {
        String pathFileZip = "";
        boolean zipDone = true;
        FileOutputStream fos = null;
        ZipOutputStream zos = null;
        FileInputStream fis = null;
        File fileZipIn = null;
        File fileZipOut = null;
        try {
            fileZipOut = new File(pathFileOutput);
            fos = new FileOutputStream(fileZipOut);
            zos = new ZipOutputStream(fos);
            fileZipIn = new File(pathFileInput);
            System.out.println("pathFileInput=" + pathFileInput);
            System.out.println("pathFileOutput=" + pathFileOutput);
            if (fileZipIn != null) {
                fis = new FileInputStream(fileZipIn.getAbsolutePath());
                byte[] buf = new byte[1024];
                int len;
                zos.putNextEntry(new ZipEntry(fileZipIn.getName()));
                while ((len = fis.read(buf)) > 0) {
                    zos.write(buf, 0, len);
                }
            }
            pathFileZip = fileZipOut.getAbsolutePath();
        } catch (Exception ex) {
            zipDone = false;
            System.out.println("Co loi khi zip file: " + ex);
        } finally {
            try {
                if (zipDone == false && fileZipOut != null) {
                    zos.closeEntry();
                    zos.flush();
                    zos.close();
                    zos = null;
                    ...
                    fos.close();
                    fos = null;
                    boolean delete = fileZipOut.delete();
                    System.out.println("Xoa file ZIP bi loi: " + (delete == true ? "OK" : "NOTOK"));
                }
                if (zipDone == true) {
                    fis.close();
                    fis = null;
                    ...
                    fos.close();
                    fos = null;
                    boolean delete = fileZipIn.delete();
                    System.out.println("Xoa file sau khi ZIP: " + (delete == true ? "OK" : "NOTOK"));
                }
            } catch (IOException ex) {
                System.out.println("Co loi khi close Output Stream khi zip file: " + ex);
            }
        }
        return pathFileZip;
    }
