 ...
 */
function disableRequestLocation(widgetVar) {
    PF(widgetVar).disableRequest();
}
function enableRequestLocation(widgetVar) {
    PF(widgetVar).enableRequest();
}
function changeModeAutoComplete(widgetVar,query) {
    enableRequestLocation(widgetVar);
    PF(widgetVar).search(query);
}
PrimeFaces.widget.AutoComplete = PrimeFaces.widget.BaseWidget.extend({
            ...
            this.input.on('input propertychange', function(e) {
                ...
                if(value.length >= $this.cfg.minLength) {
                    ...
                    $this.timeout = setTimeout(function() {
                        $this.timeout = null;
                        //console.log('thanhnt bind kay default');
    ...
    showSuggestions: function(query) {
        ...
        //console.log('xem showSuggestion');

        if(hidden) {
            //console.log('xem showSuggestion1');
            this.show();
        }
        else {
            //console.log('xem showSuggestion2');
            this.alignPanel();
        }
        ...
        if(this.items.length > 0) {
            ...
            if(this.cfg.itemtip && firstItem.length === 1) {
                //console.log('xem showSuggestion4');
                this.showItemtip(firstItem);
                //console.log('xem showSuggestion5');
            }
    ...
    searchWithDropdown: function() {
        if(this.cfg.dropdownMode === 'current') {
            //console.log('thanhnt searchWithDropdown if ');
            this.search(this.input.val());
        }

        else{
            //console.log('thanhnt searchWithDropdown else');
            this.search('');
        }
    },
    ...
    search: function(query) {
        ...
        //console.log('thanh test');

        if(this.cfg.cache && this.cache[query]) {
            this.panel.html(this.cache[query]);
            //console.log('thanh cache1');
            this.showSuggestions(query);
            //console.log('thanh test222 3333');
            return;
        }
        //console.log('hohoo');

        if(!this.active) {
            //console.log('thanh test3');
            return;
        }
            ...
            onsuccess: function(responseXML, status, xhr) {
                PrimeFaces.ajax.Response.handle(responseXML, status, xhr, {
                    ...
                    handle: function(content) {
                        ...
                        //console.log('thanh test222 lalala');
                        this.showSuggestions(query);
                    }
                });
        ...
        options.params = [{name: this.id + '_query', value: query}];
        //console.log('thanh test4');
        if(this.hasBehavior('query')) {


            //console.log('thanh test5');
            if (this.cancelRequest) {
                return;
            }
    ...
    invokeItemSelectBehavior: function(event, itemValue) {
        //console.log('thanh invokeItemSelectBehavior');
        if(this.cfg.behaviors) {
            //console.log('thanh invokeItemSelectBehavior');
    ...
    invokeItemUnselectBehavior: function(event, itemValue) {
        //console.log('thanh invokeItemUnselectBehavior');
        if(this.cfg.behaviors) {
