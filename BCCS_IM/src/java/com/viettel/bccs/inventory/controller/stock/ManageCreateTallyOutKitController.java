...
@Component
@Scope("view")
@ManagedBean(name = "manageCreateTallyOutKitController")
public class ManageCreateTallyOutKitController extends TransCommonController {
    ...
    private void initProduct() {
        try {
            ...
            fieldExportFileDTOs = new ArrayList<>();
            initTagProduct();
    ...
    public void downloadFileTemplate() {
        try {
            InputStream createStream = Faces.getResourceAsStream(Const.BRAS_IPPOOL.RESOURCE_TEMPLATE_PATH + "stock_trans_template/KitExchangeFilePattern.xls");
            HSSFWorkbook workbook = new HSSFWorkbook(createStream);
            FacesContext facesContext = FacesContext.getCurrentInstance();
            ExternalContext externalContext = FacesContext.getCurrentInstance().getExternalContext();
            externalContext.setResponseHeader("Content-Disposition", "attachment; filename=\"" + "KitExchangeFilePattern.xls" + "\"");
        ...
        try {
            ...
            if (DataUtil.isNullOrEmpty(fieldExportFileDTOs)) {
                ...
                if (!DataUtil.isNullOrEmpty(lstErrFieldExport)) {
                    clearUpload();
                    reportWarn("", "mn.list.create.tally.out.kit.file.result.reup", DataUtil.safeToString(fieldExportFileDTOs.size()),
...
//            return fileExportBean.getExportFileContent();
    ...
    public StreamedContent downloadFileError() {
        try {
            FileExportBean fileExportBean = new FileExportBean();
            fileExportBean.setOutName("Kit_CreateTallyOutFileErr_" + staffDTO.getStaffCode() + ".xls");
            fileExportBean.setTempalatePath(FileUtil.getTemplatePath());
            fileExportBean.setOutPath(FileUtil.getOutputPath());
            fileExportBean.setTemplateName("Kit_ExchangeFilePattern.xls");
...
//            return fileExportBean.getExportFileContent();
    ...
    private void validateFileUpload(ExcellUtil processingFile) {
        try {
            ...
            for (int i = 1; i < totalRow; i++) {
                ...
                String prodOfferCodeNew = processingFile.getStringValue(row.getCell(j++)).trim();
                ...
                String stateId = processingFile.getStringValue(row.getCell(j++)).trim();
                String strQuantity = processingFile.getStringValue(row.getCell(j++)).trim();
                String note = processingFile.getStringValue(row.getCell(j)).trim();

                dto.setFromOwnerCode(fromOwnerCode);
                dto.setProductOfferCode(productOfferCode);
                dto.setProdOfferCodeNew(prodOfferCodeNew);
                dto.setFromSerial(fromSerial);
                dto.setToSerial(toSerial);
                dto.setStrState(stateId);
                dto.setStrQuantity(strQuantity);
                dto.setNote(note);
                dto.setStateName(productOfferingService.getStockStateName(DataUtil.safeToLong(stateId)));
                dto.setStateId(DataUtil.safeToLong(stateId));
    ...
    private String validateRow(FieldExportFileDTO fieldExportFileDTO, List<FieldExportFileDTO> fieldExportFileDTOs) throws Exception {
        ...
        String prodOfferCodeNew = fieldExportFileDTO.getProdOfferCodeNew();
...
//        }
        ...
        if (!DataUtil.safeEqual(fromOwnerCode, staffDTO.getShopCode())) {
        ...
        if (!Const.STATE_STATUS.NEW.equals(stateId)
                && !Const.STATE_STATUS.DAMAGE.equals(stateId)
                && !Const.STATE_STATUS.RETRIEVE.equals(stateId)) {
            return getText("mn.isdn.manage.create.field.validate.stateId");
        }
        ...
        if (DataUtil.isNullObject(prodOfferCodeNew)) {
            return getText("mn.isdn.manage.create.field.validate.productOffer");
        }
...
//            }
        ...
        ProductOfferingDTO dtoNew = productOfferingService.findByProductOfferCode(prodOfferCodeNew, Const.STATUS.ACTIVE);
        if (DataUtil.isNullObject(dtoNew) || !DataUtil.safeEqual(dtoNew.getStatus(), Const.STATUS.ACTIVE)) {
            return getText("mn.isdn.manage.creat.field.export.productOffer.active");
        } else {
            fieldExportFileDTO.setProductOfferIdNew(dtoNew.getProductOfferingId());
            fieldExportFileDTO.setProductOfferingDTONew(dtoNew);
            fieldExportFileDTO.setProdOfferNameNew(dtoNew.getName());
        }
        ...
        boolean check = stockSimService.checkSimElegibleExists(fromSerial, toSerial);
        if (!check) {
            return getText("mn.list.create.tally.out.kit.file.result.sim");
        }
        BaseMessage baseMess = saleWs.checkActiveSerial(fromSerial, toSerial);
        if (!baseMess.isSuccess()) {
            return getText("mn.list.create.tally.out.kit.file.result.sim.isdn");
        }
    ...
    public void doCreateNote() {
        try {
            int x = fieldExportFileDTOs.size();
            int y = lstErrFieldExport.size();

            if (!DataUtil.isNullOrEmpty(lstErrFieldExport)) {
                clearUpload();
                fieldExportFileDTOs = new ArrayList<>();
                reportErrorValidateFail("", "", "mn.list.create.tally.out.kit.file.result.reup", DataUtil.safeToString(x),
                        DataUtil.safeToString(x + y));
                return;
            }
            ...
            Map<Long, StockTransDetailDTO> mapDetail = Maps.newHashMap();

            for (FieldExportFileDTO exportDTO : fieldExportFileDTOs) {
                Long quantity;
                if (!mapDetail.containsKey(exportDTO.getProductOfferId())) {
                    ...
                    serialDTO.setFromSerial(exportDTO.getFromSerial());
                    serialDTO.setToSerial(exportDTO.getToSerial());
                    serialDTO.setQuantity(exportDTO.getQuantity());
                    serialDTO.setStateId(exportDTO.getStateId());
                    stockTransDetailAdd.setLstStockTransSerial(Lists.newArrayList(serialDTO));
                    quantity = exportDTO.getQuantity();
                    stockTransDetailAdd.setQuantity(quantity);

                    mapDetail.put(exportDTO.getProductOfferId(), stockTransDetailAdd);
                } else {
                    StockTransDetailDTO detail = mapDetail.get(exportDTO.getProductOfferId());
                    List<StockTransSerialDTO> lsSErial = detail.getLstSerial();
                    StockTransSerialDTO serialDTO = new StockTransSerialDTO();
                    serialDTO.setFromSerial(exportDTO.getFromSerial());
                    serialDTO.setToSerial(exportDTO.getToSerial());
                    serialDTO.setQuantity(exportDTO.getQuantity());
                    serialDTO.setStateId(exportDTO.getStateId());
                    lsSErial.add(serialDTO);
                    quantity = DataUtil.defaultIfNull(detail.getQuantity(), 0L) + serialDTO.getQuantity();
                    detail.setQuantity(quantity);
                }

            }
            List<StockTransDetailDTO> lsDetailDTOs;
            lsDetailDTOs = new ArrayList<>(mapDetail.values());
    ...
    private StockTransDTO getStockTransExport() {
        ...
        stockTransExport.setActionCode(transInputDTO.getActionCode());
