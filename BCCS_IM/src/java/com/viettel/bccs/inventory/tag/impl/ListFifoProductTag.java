...
@Service
@Scope("prototype")
public class ListFifoProductTag extends InventoryController implements ListFifoProductNameable {
    ...
    private ListProductOfferDTO createNewListProOffer() throws LogicException, Exception {
        ...
        if (DataUtil.isNullOrEmpty(lsProductOfferTypeDTOTmp)) {
            if (DataUtil.safeEqual(this.ownerType, Const.OWNER_TYPE.STAFF)) {
                ...
                if (staffDTO != null) {
                    if (Const.GOODS_STATE.RESCUE.equals(configTagDto.getStateId())) {
...
//            return fileExportBean.getExportFileContent();
    ...
    private void createStockDetailProductdByFile(ExcellUtil processingFile) throws LogicException, Exception {
        ...
        for (Row row : sheetProcess) {
            ...
            //2.2validate trang thai chi chap nhan cac gia tri nam trong dach sach trang thai cau hinh
            ...
            if (Const.STOCK_TYPE.STOCK_HANDSET.equals(prodOfferTypeDto.getProductOfferTypeId())) {
                ...
                for (int j = 0; j < lsStockFullFile.size(); j++) {
                    ...
                    StockTransFullDTO stockTmp = lsStockFullFile.get(j);
                    boolean isContinue = DataUtil.isNullOrEmpty(stockTmp.getTableName()) || Const.STOCK_TYPE.STOCK_HANDSET.equals(stockTmp.getProductOfferTypeId())
...
//                errorImport = true;
    ...
    public void doSelectSerial() {
        try {
            ...
            String serialInvalidFifo = validateFifoSerial();
            if (!DataUtil.isNullOrEmpty(serialInvalidFifo)) {
                reportWarn("", "stock.input.serial.fifo.notvalid");
            }
    ...
    /**
     * view serial trong kho xuat
     *
     * @throws LogicException
     * @throws Exception
     * @author thanhnt77
     */
    private void showListSerialInStock() throws LogicException, Exception {
        ...
        listSerialView = DataUtil.defaultIfNull(stockTransSerialService.getRangeSerialFifo(DataUtil.safeToLong(stockTransDetail.getFromOwnerType()), fromOwnerId,
    ...
    /**
     * ham validate serial theo quy tac nhap truoc xuat truoc
     *
     * @throws LogicException
     * @throws Exception
     * @author thanhnt77
     */
    private String validateFifoSerial() throws LogicException, Exception {
        if (DataUtil.isNullOrEmpty(listSerialRangeSelect)) {
            return "";
        }
        ...
        Long quantity = DataUtil.safeToLong(stockTransDetail.getQuantity());//so luong yeu cau
        List<StockTransSerialDTO> listSerialViewValid = Lists.newArrayList();
        Date firstDate = listSerialView.get(0).getCreateDatetime();
        StringBuilder serialList = new StringBuilder("");

        for (StockTransSerialDTO serialDTO : listSerialView) {
            if (firstDate.compareTo(serialDTO.getCreateDatetime()) == 0 || quantity.compareTo(0L) > 0 || quantity.compareTo(DataUtil.safeToLong(serialDTO.getQuantity())) > 0) {
                firstDate = serialDTO.getCreateDatetime();
                quantity = quantity - DataUtil.safeToLong(serialDTO.getQuantity());
                listSerialViewValid.add(serialDTO);
                continue;
            }
            break;
        }

        if (!DataUtil.isNullOrEmpty(listSerialViewValid)) {
            boolean isCheck;
            if (getHandset()) {
                for (StockTransSerialDTO serialDTO : listSerialRangeSelect) {
                    isCheck = false;
                    for (StockTransSerialDTO serialValid : listSerialViewValid) {
                        if (DataUtil.safeToString(serialDTO.getFromSerial()).equals(serialValid.getFromSerial())) {
                            isCheck = true;
                            break;
                        }
                    }
                    if (!isCheck) {
                        serialList.append(serialDTO.getFromSerial()).append("-").append(serialDTO.getToSerial()).append(";");
                        serialDTO.setNotRule(1L);
                    }
                }
            } else {
                for (StockTransSerialDTO serialDTO : listSerialRangeSelect) {
                    isCheck = false;
                    BigInteger fromSerialNumSelect = new BigInteger(serialDTO.getFromSerial());
                    BigInteger toSerialNumSelect = new BigInteger(serialDTO.getToSerial());
                    for (StockTransSerialDTO serialValid : listSerialViewValid) {
                        BigInteger fromSerialNumValid = new BigInteger(serialValid.getFromSerial());
                        BigInteger toSerialNumValid = new BigInteger(serialValid.getToSerial());
                        if (fromSerialNumValid.compareTo(fromSerialNumSelect) <= 0 && toSerialNumSelect.compareTo(toSerialNumValid) <= 0) {
                            isCheck = true;
                            break;
                        }
                    }
