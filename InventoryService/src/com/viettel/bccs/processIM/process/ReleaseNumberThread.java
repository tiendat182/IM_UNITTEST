 ...
 */
public class ReleaseNumberThread extends InventoryThread {
    private Long batch;
    ...
    public ReleaseNumberThread(String name, int numActorLimit, int type) {
    ...
    public void execute() {
        try {
            reportError("------Bat dau tien trinh giai phong so thuong ");
            List<Long> lstReleaseShopId = Lists.newArrayList();
            keepTimeHour = DataUtil.safeToDouble(optionSetValueService.getValueByTwoCodeOption(Const.REVOKE_NUMBER.TIME_LOCK_NUMBER, Const.REVOKE_NUMBER.TIME_LOCK_NUMBER));
            if (DataUtil.isNullObject(keepTimeHour) || DataUtil.safeEqual(keepTimeHour, 0.0)) {
                reportError("------Chua cau hinh thoi gian giai phong so thuong --- ");
                return;
            }
            String lstShopCode = optionSetValueService.getValueByTwoCodeOption(Const.REVOKE_NUMBER.STOCK_LOCK_NUMBER, Const.REVOKE_NUMBER.STOCK_LOCK_NUMBER);
            if (DataUtil.isNullOrEmpty(lstShopCode)) {
                reportError("------Chua cau hinh kho giai phong so thuong --- ");
                return;
            }
            ...
            reportError("------Giai phong thanh cong " + count + "/" + lstRelease.size() + " so thuong");
        } catch (Exception ex) {
            reportError(ex);
        }
    }
